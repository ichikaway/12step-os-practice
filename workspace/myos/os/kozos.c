...
{
    kz_msgbuf *msgbp;
    msgbp = (kz_msgbuf *) kzmem_alloc(sizeof(*msgbp));
    if (msgbp == NULL) {
        kz_sysdown();
    }
    msgbp->next = NULL;
    msgbp->sender = thp;
    msgbp->param.size = size;
    msgbp->param.p = p;

    if(mboxp->tail) {
        mboxp->tail->next = msgbp;
    } else {
        mboxp->head = msgbp;
    }
    mboxp->tail = msgbp;
...
{
    kz_msgbuf *mp;
    kz_syscall_param_t *syscall_param_p;
    ...
    syscall_param_p = mboxp->receiver->syscall.param;
    syscall_param_p->un.recv.ret = (kz_thread_id_t) mp->sender;
    if (syscall_param_p->un.recv.sizep) {
        *(syscall_param_p->un.recv.sizep) = mp->param.size;
    }
    if (syscall_param_p->un.recv.pp) {
        *(syscall_param_p->un.recv.pp) = mp->param.p;
    }
...
static kz_thread_id_t thread_recv(kz_msgbox_id_t id, int *size, char **pp)
{
    kz_msgbox *mboxp = &msgboxes[id];

    if(mboxp->receiver) { //他のスレッドが受信待ちしている
        kz_sysdown();
    }
    mboxp->receiver = current;

    if(mboxp->head == NULL) {
        //メッセージボックスにメッセージがないのでスレッドをスリープさせる。
        // システムコールがブロックしたまになりスリープとなる
        // システムコール処理の先頭でgetcurrent()によりスレッドをレディキューから抜き出しているため
        // そのままreturnで戻るとスレッドはスリープとなる
        return -1;
    }
